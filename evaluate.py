...
def write_metrics(output_folder, all_metrics, opt):
    jpegcompr = opt.jpegCompression
    transformiter = opt.transformIterations
    ...
    with open( os.path.join(output_folder, f'ap_{jpegcompr}_{transformiter}.txt'), 'a') as f:
    ...
    with open( os.path.join(output_folder, f'acc_05_{jpegcompr}_{transformiter}.txt'), 'a') as f:
    ...
    with open( os.path.join(output_folder, f'acc_oracle_{jpegcompr}_{transformiter}.txt'), 'a') as f:
...
def run_for_model(datasets, model, opt):
    ...
    write_metrics(output_folder, all_metrics, opt)
